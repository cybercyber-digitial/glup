...
describe('glup.dest()', function() {
  beforeEach(rimraf.bind(null, outpath));
  afterEach(rimraf.bind(null, outpath));

  it('should return a stream', function(done) {
    var stream = glup.dest(path.join(__dirname, './fixtures/'));
  ...
  it('should return a output stream that writes files', function(done) {
    var instream = glup.src('./fixtures/**/*.txt', { cwd: __dirname });
    var outstream = glup.dest(outpath);
  ...
  it('should return a output stream that does not write non-read files', function(done) {
    var instream = glup.src('./fixtures/**/*.txt', { read: false, cwd: __dirname });
    var outstream = glup.dest(outpath);
    instream.pipe(outstream);
  ...
  it('should return a output stream that writes streaming files', function(done) {
    var instream = glup.src('./fixtures/**/*.txt', { buffer: false, cwd: __dirname });
    var outstream = instream.pipe(glup.dest(outpath));
  ...
  function testWriteDir(srcOptions, done) {
    var instream = glup.src('./fixtures/stuff', srcOptions);
    var outstream = instream.pipe(glup.dest(outpath));
