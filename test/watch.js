...
describe('glup.watch()', function() {
  ...
  it('should call the function when file changes: no options', function(done) {
    ...
    var watcher = glup.watch('watch-func.txt', { cwd: outpath }, function(cb) {
      watcher.close();
      cb();
      done();
    });
  ...
  it('should execute the glup.parallel tasks', function(done) {
    var tempFile = path.join(outpath, 'watch-func.txt');

    createTempFile(tempFile);

    glup.task('test', function(cb) {
      watcher.close();
      cb();
      done();
    });

    var watcher = glup.watch('watch-func.txt', { cwd: outpath }, glup.parallel('test'));
  ...
  it('should work with destructuring', function(done) {
    var tempFile = path.join(outpath, 'watch-func.txt');
    var watch = glup.watch;
    var parallel = glup.parallel;
    var task = glup.task;
    createTempFile(tempFile);
  ...
  it('should not call the function when no file changes: no options', function(done) {
    ...
    var watcher = glup.watch('watch-func.txt', { cwd: outpath }, function() {
  ...
  it('should call the function when file changes: w/ options', function(done) {
    ...
    var watcher = glup.watch('watch-func-options.txt', { cwd: outpath }, function(cb) {
      watcher.close();
      cb();
      done();
    });
  ...
  it('should not drop options when no callback specified', function(done) {
    ...
    var watcher = glup.watch(relFile, { cwd: cwd })
  ...
  it('should run many tasks: w/ options', function(done) {
    ...
    glup.task('task1', function(cb) {
      a++;
      cb();
    });
    glup.task('task2', function(cb) {
      a += 10;
      expect(a).toEqual(11);
      watcher.close();
      cb();
    ...
    var watcher = glup.watch('watch-task-options.txt', { cwd: outpath }, glup.series('task1', 'task2'));
  ...
  it('should run many tasks: no options', function(done) {
    ...
    var watcher = glup.watch('./test/out-fixtures/watch-many-tasks-no-options.txt', glup.series('task1', 'task2'));
  ...
  it('should throw an error: passed parameter (string) is not a function', function(done) {
    ...
    createTempFile(tempFile);
    try {
      glup.watch(filename, { cwd: outpath }, 'task1');
    } catch (err) {
      expect(err.message).toEqual('watching ' + filename +  ': watch task has to be a function (optionally generated by using glup.parallel or glup.series)');
      done();
    }
  });
