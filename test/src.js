...
describe('glup.src()', function() {
  it('should return a stream', function(done) {
    var stream = glup.src('./fixtures/*.coffee', { cwd: __dirname });
  ...
  it('should return a input stream for multiple globs', function(done) {
    ...
    var stream = glup.src(globArray, { cwd: __dirname });
  ...
  it('should return a input stream with no contents when read is false', function(done) {
    var stream = glup.src('./fixtures/*.coffee', { read: false, cwd: __dirname });
    stream.on('error', done);
    stream.on('data', function(file) {
      expect(file).toExist();
      expect(file.path).toExist();
  ...
  it('should return a input stream with contents as stream when buffer is false', function(done) {
    var stream = glup.src('./fixtures/*.coffee', { buffer: false, cwd: __dirname });
    stream.on('error', done);
    stream.on('data', function(file) {
      expect(file).toExist();
      expect(file.path).toExist();
  ...
  it('should return a input stream from a deep glob', function(done) {
    var stream = glup.src('./fixtures/**/*.jade', { cwd: __dirname });
    stream.on('error', done);
    stream.on('data', function(file) {
      expect(file).toExist();
      expect(file.path).toExist();
  ...
  it('should return a input stream from a deeper glob', function(done) {
    var stream = glup.src('./fixtures/**/*.dmc', { cwd: __dirname });
    var a = 0;
    stream.on('error', done);
  ...
  it('should return a file stream from a flat path', function(done) {
    var a = 0;
    var stream = glup.src(path.join(__dirname, './fixtures/test.coffee'));
    stream.on('error', done);
    stream.on('data', function(file) {
      ++a;
      expect(file).toExist();
